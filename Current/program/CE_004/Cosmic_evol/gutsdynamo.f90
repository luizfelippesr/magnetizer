...
module direc_names
  implicit none
!
  character (len=6), parameter :: s0= 'CE_004'
...
!
  ...
  contains
    subroutine set_input_params(gal_id_string)
!
...
!    DIMENSIONAL INPUT PARAMETERS THAT MUST BE SPECIFIED AND MAY VARY FROM GALAXY TO GALAXY AND FROM TIMESTEP TO TIMESTEP
      if (Read_param) then
        open(29,file= s1 // '/fortran_pde/1D/telegraph/r_noz/input/' // s0 // '/time_indep_params_' &
        ...
        open(30,file= s1 // '/fortran_pde/1D/telegraph/r_noz/input/' // s0 // '/time_dep_params_'   &
                      // gal_id_string // '.in',status="old")
        if (iread == 0) then
...
!  NUMERICS
  ...
  integer, parameter :: n2= 20!20!80!3600  !long(1./dt)+1;	!Dump output n1 times (with n2 timesteps between each dump)
  integer, parameter :: nread= 1680                                !Read in new input parameters every nread timesteps
  integer, parameter :: nscreen= 60                                !Print output to screen every nscreen*n2 timesteps
...
!
contains
  subroutine construct_galaxy_model(gal_id_string)
!
    character (len=8), intent(in) :: gal_id_string
!
...
!
    if (Time_evol) then
      call set_input_params(gal_id_string)  !read in model parameters
...
!
contains
  subroutine init_start(gal_id_string)
!
    character (len=8), intent(in) :: gal_id_string
!
...
!
    ...
    close(20)
    open(10,file= s1 // '/fortran_pde/1D/telegraph/r_noz/output/' // s0 &
    ...
    print*,'Writing initial output to file init.out'
    open(20,file= 'diagnostic.out',status="old",position="append")
    write(20,*)'Finished writing parameters to file param.out'
    write(20,*)'Writing initial output to file init.out'
    write(20,*)''
    close(20)
    open(11,file= s1 // '/fortran_pde/1D/telegraph/r_noz/output/' // s0 &
    ...
    write(11,*)Bzmod
    close(11)
    print*,'Finished writing initial output to file init.out'
    print*,''
    open(20,file= 'diagnostic.out',status="old",position="append")
    write(20,*)'Finished writing initial output to file init.out'
    close(20)
...
!  
      print*,'Writing output for final timestep to file run.out'
      open(20,file= 'diagnostic.out',status="old",position="append")
      write(20,*)'Writing output for final timestep to file run.out'
      close(20)
      open(12,file= s1 // '/fortran_pde/1D/telegraph/r_noz/output/' // s0 &
...
!  
      print*,'Writing time series output to file ts.out'
      open(13,file= s1 // '/fortran_pde/1D/telegraph/r_noz/output/' // &
