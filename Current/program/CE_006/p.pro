...
for iplot=0,1 do begin
  if (iplot eq 0) then begin
    ...
    window,0,xsize=xsize,ysize=ysize,xpos=xpos,ypos=ypos
  endelse
  ;
  xr=[0,r_disk_kpc]
  yr=[(min(ts_Bp_mkG[nt,nxghost:nx-1-nxghost])<min(ts_Br_mkG[nt,nxghost:nx-1-nxghost]))-0.05*(max(ts_Bp_mkG[nt,nxghost:nx-1-nxghost])>max(ts_Br_mkG[nt,nxghost:nx-1-nxghost])),1.05*(max(ts_Bp_mkG[nt,nxghost:nx-1-nxghost])>max(ts_Br_mkG[nt,nxghost:nx-1-nxghost]))]
  xticks=3 & xminor=5
  yticks=4 & yminor=5
  ;
  ...
  oplot,r_kpc[nxghost:nx-1-nxghost],ts_Bp_mkG   [ntfrac*nt,nxghost:nx-1-nxghost],li=0,th=th,col=col2
  oplot,r_kpc[nxghost:nx-1-nxghost],ts_Br_mkG   [ntfrac*nt,nxghost:nx-1-nxghost],li=2,th=th,col=col2
  oplot,r_kpc[nxghost:nx-1-nxghost],ts_Bzmod_mkG[ntfrac*nt,nxghost:nx-1-nxghost],li=3,th=th,col=col2
  ...
  ;
  xyouts,0.5*xr[1],0.6*yr[1],'t(Gyr)='           ,col=col
  xyouts,0.6*xr[1],0.6*yr[1],ts_t_Gyr[       nt],col=col
  xyouts,0.5*xr[1],0.4*yr[1],'t(Gyr)='           ,col=col2
  xyouts,0.6*xr[1],0.4*yr[1],ts_t_Gyr[ntfrac*nt],col=col2
endfor
;
;2) alp_m, alp_k vs r
for iplot=0,1 do begin
  ...
  ;
  xr=[0,r_disk_kpc]
  yr=[(min(ts_alp_k_kms[nt,nxghost:nx-1-nxghost])<min(ts_alp_m_kms[nt,nxghost:nx-1-nxghost]))-0.05*(max(ts_alp_k_kms[nt,nxghost:nx-1-nxghost])>max(ts_alp_m_kms[nt,nxghost:nx-1-nxghost])),1.05*(max(ts_alp_k_kms[nt,nxghost:nx-1-nxghost])>max(ts_alp_m_kms[nt,nxghost:nx-1-nxghost]))]
  xticks=3 & xminor=5
  yticks=4 & yminor=5
  ;
...
for iplot=0,1 do begin
  if (iplot eq 0) then begin
    ...
    xsize=10. & ysize=6.
    ntstr=strjoin(['nt',strtrim(string(nt),2)])
    device,/encapsul,filename=strjoin([s0,s2,'_p_',ntstr,'.eps']),xsize=xsize,ysize=ysize;,bits=8;,/color
    char=1.5
    th=1.
    !p.charsize=char
  endif else begin
  ...
  oplot,[t1_Gyr,t1_Gyr],[1.d-5,1.d5],th=th,li=1
  oplot,[t2_Gyr,t2_Gyr],[1.d-5,1.d5],th=th,li=1
  oplot,[0,100000],[gamma_mean_Gyr,gamma_mean_Gyr],th=th,li=1
  ;
  axis,xaxis=0,xth=th,xticklen=0.03,xtickname=replicate(' ',30);,xticks=xticks,xminor=xminor
  axis,xaxis=1,xth=th,xticklen=0.03,xtickname=replicate(' ',30);,xticks=xticks,xminor=xminor
  axis,yaxis=0,yth=th              ,ytickname=replicate(' ',30);,yticks=yticks,yminor=yminor
