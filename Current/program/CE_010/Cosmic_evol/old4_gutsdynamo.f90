...
! ALPHA QUENCHING
  logical, parameter :: Dyn_quench=   .true.  !Works with alg_quench=F; Set to T for dynamical quenching (d_alpha_m/dt eqn incl in sim)
...
!mark1
        Uz_sol_kms=     10.0d0  !Vertical mean velocity at r=r_sol in km/s (if Var_Uz=F then Uz_kms=Uz_sol_kms)
...
!!        print*, 'delta_r_B=',delta_r_B*r_disk_kpc
        ...
        Bp_floor=Bp_floor*r**(1.d0/2)*exp(-(r-rmax)**2/2/(delta_r/2)**2)  !multiply by r^(1/2)*(renormalized Gaussian of width delta_r/2)
!mark2
...
!        alp(nxghost+2:nx-nxghost)= alp(nxghost+2:nx-nxghost)* &
!                                  (1.d0 +Bp_floor(nxghost+2:nx-nxghost)**2 &
!                                         /Bsqtot(nxghost+2:nx-nxghost))  !Formula for simple alpha quenching
!        alp(ifloor1:ifloor2)= alp(ifloor1:ifloor2)* &
!                              (1.d0 +Bp_floor(ifloor1:ifloor2)**2/Bsqtot(ifloor1:ifloor2))  !Formula for simple alpha quenching
!        print*, alp/alp_k
