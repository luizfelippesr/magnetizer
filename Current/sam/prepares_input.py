...
def density(r_50, M_stars, M_gas, vt, alpha=4.0):
    ...
    n_sol = cq.density_cgs(0, r_50, M_stars, M_gas, sigma=vt, alpha=4.0)/m
    r_n = r_50/3.36
...
def height(r_50, M_stars, M_gas, vt, alpha=4.0):
    """ Computes the density at a given radius in cm^{-3}.
        ...
        WARNING This is valid if vt is a constant...
    """
    h_sol = cq.scaleheight_kpc(0, r_50, M_stars, M_gas, sigma=vt, alpha=4.0)
    r_n = r_50/1.68
...
if __name__ == "__main__"  :
    ...
    for ID in IDs:
        filename = 'time_dep_params_{0}.in'.format(ID)
        header =('time        |'
                 'l_sol_kpc   |'
                 'r_l_kpc     |'
                 'v_sol_kms   |'
