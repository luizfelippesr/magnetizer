...
module direc_names  !Specifies paths
  implicit none
!
  character (len=6), parameter :: s0= 'CE_006'
...
!
  implicit none
!
  integer, parameter :: nxphys= 31!61!91  !Resolution in r (excluding ghost zones) (for convenience should be N*r_disk_kpc+1)
...
!
  implicit none
!
  integer, parameter :: n1= 420   !Number of snapshots
  integer, parameter :: nread= 99999  !Read in new input parameters every nread timesteps
  integer, parameter :: nscreen= 420  !Print output to screen every nscreen*n2 timesteps
  ...
  double precision :: dt,t=0.d0,first=0.d0, eps_t=0.01d0
!
  contains
