...
class AverageProfiles(object):
    """
    Stores the profile data for a given quantity.

    Will compute the media profiles as well as 15th and 85 percentiles.
    ...
    Note
    ----
    For future: eventually, a general PercentileData class could be written
    and both AverageProfiles and TimeEvolution could be derived classes.

    """
    def __init__(self, quantity, mag_run, bin_objs=None,
                 target_redshifts=None, minimum_number_per_bin=5,
                 ignore_zero_valued=False, log=False, log0=None,
        ...
        self.lower = np.empty((self.nbins, self.zs.size, self.ngrid))*np.nan
        self.upper = np.empty((self.nbins, self.zs.size, self.ngrid))*np.nan
        self.med = np.empty((self.nbins, self.zs.size, self.ngrid))*np.nan
        self.ngals = np.empty((self.nbins, self.zs.size, self.ngrid))*np.nan
        ...
        for j, z in enumerate(self.zs):
            if no_binning:
                ...
                if single_binning:
                    zdata = mag_run.get(quantity, z=z,
                                        cache_intermediate=cache_intermediate,
                                        cache=cache, binning=bin_objs)
                else:
                    zdata = mag_run.get(quantity, z=z,
                                        cache_intermediate=cache_intermediate,
                                        cache=cache, binning=bin_dict[z])

            for i in range(self.nbins):
                data = zdata[i]
                # Checks whether it has units and strips them away
                self.unit, data = get_formated_units(data, return_base=True,
                                                     clean=log)
                for ir in range(self.ngrid):
                    ...
                    self.lower[i][j][ir], self.med[i][j][ir], self.upper[i][j][ir] = \
                                                np.percentile(datum, percentiles)
                    if log:
                        for p in (self.lower, self.med, self.upper):
                            p[i][j][ir] = np.log10(p[i][j][ir])

                    self.ngals[i][j][ir] = datum.size
